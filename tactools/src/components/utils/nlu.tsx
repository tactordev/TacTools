...
const PREFIX_WORDS = new Set([
    "due", "on", "by", "for", "before", "until", "in", "within", "from", "starting", "start"
]);
...
export function nlu(text: string): NluResult {
    ...
    for (let count = 0; count < words.length; count++) {
        ...
        if (isTimeContext) {
            ...
            if (word === "morning") { target.setHours(9, 0, 0, 0); matched = true; }
            if (word === "afternoon") { target.setHours(14, 0, 0, 0); matched = true; }
            if (word === "evening") { target.setHours(17, 30, 0, 0); matched = true; }
            if (word === "night") { target.setHours(21, 0, 0, 0); matched = true; }
            if (word === "noon") { target.setHours(12, 0, 0, 0); matched = true; }
            if (word === "midnight") { target.setHours(23, 59, 59, 999); matched = true; }
