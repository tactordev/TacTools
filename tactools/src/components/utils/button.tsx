...
export default function Button({
  ...
  return (
    <div
      ref={button}
      className={`group hover:cursor-pointer hover:bg-blue-200/30 scale-100 hover:scale-105 px-2 py-1 rounded-md shadow-sm transition-all duration-200 select-none ${className}`}
      onClick={onClick ?? foo}
      onBlur={onBlur ?? foo}
      title={name ?? "Button"}
