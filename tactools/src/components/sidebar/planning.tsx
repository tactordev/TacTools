...
function CTXMenu({
  ...
  return (
    <ContextMenu x={x} y={y} onBlur={onBlur}>
      ...
      {!["overview", "calendar", "study tracker"].includes(
...
export default function Calendar({
  ...
  const [lists, setLists] = useState<{ name: string; id: number }[]>(() => {
    if (typeof window === "undefined")
      return [
        ...
        { name: "calendar", index: 0 },
        { name: "study tracker", id: 0 },
      ];
  ...
  useEffect(() => {
    const parsed = {
      values: lists.filter(
        (value) =>
          !["overview", "calendar", "study tracker"].includes(value.name),
      ),
    };
          ...
          {lists.map((value: { name: string; id: number }, index: number) => {
            return (
              <div key={`list-${value.id}`}>
                ...
                >
                  <Button
                    className="flex flex-row items-center gap-2 mt-1 !shadow-none !py-0 has-[:focus]:bg-blue-200/30"
                    name={title(lists[index].name)}
                    onClick={(e) => {
                        ...
                        setTabs([
                          ...previous,
                          {
                            ...
                            value: {
                              icon:
                                value.name === "overview" ? (
                                  <Inbox className="w-4 h-4 text-gray-600/80" />
                                ) : value.name === "calendar" ? (
                                  <Calendar1 className="w-4 h-4 text-gray-600/80" />
                                ) : value.name === "study tracker" ? (
                                  <Timer className="w-4 h-4 text-gray-600/80" />
                                ) : (
                  ...
                  >
                    {value.name === "overview" ? (
                      ...
                      <Hash className="w-4 h-4 text-gray-600/80" />
                    )}
                    {editing === index ? (
                      <form
                        onClick={(e) => {
