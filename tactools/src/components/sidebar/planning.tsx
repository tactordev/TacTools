...
import { Hash, Plus, Inbox, Edit, Trash2, Calendar1, Timer } from "lucide-react";
...
function CTXMenu(
    ...
    return (
        <ContextMenu x={x} y={y} onBlur={onBlur}>
            ...
            { !["overview", "calendar", "study tracker"].includes(lists.find((list) => list.id === id)!.name) ? (
                <Button onClick={() => {
...
export default function Calendar({ tabs, setTabs }: { tabs: Tab[]; setTabs: (tabs: Tab[]) => void; }) {
    ...
    const [lists, setLists] = useState<{ name: string; id: number; }[]>(() => {
        if (typeof window === "undefined") return [{ name: "overview", index: -1 }, { name: "calendar", index: 0 }, { name: "study tracker", id: 0 }];

        const saved = localStorage.getItem("listNames");
        if (saved) return [ { name: "overview", id: -2 }, { name: "calendar", id: -1 }, { name: "study tracker", id: 0 }, ...JSON.parse(saved).values ];

        return [{ name: "overview", id: -2 }, { name: "calendar", id: -1 }, { name: "study tracker", id: 0 }];
    });
    ...
    useEffect(() => {
        const parsed = {
            values: lists.filter((value) => !["overview", "calendar", "study tracker"].includes(value.name))
        };
                                ...
                                <div key={`list-${value.id}`}>
                                    <motion.div  className="flex flex-col relative" id={`motiondiv-${index}`} onContextMenu={ (e: React.MouseEvent) => { rightClick(e, index); } } onDoubleClick={ editName } initial={{ opacity: 0, translateX: -5 }} animate={{ opacity: 1, translateX: 0 }} transition={{ type: "tween", delay: Math.min((index + 1)*0.015, 0.2), duration: 0.15 }} >
                                        <Button  className="flex flex-row items-center gap-2 mt-1 !shadow-none !py-0 has-[:focus]:bg-blue-200/30" name={title(lists[index].name)} onClick={(e) => {
                                            ...
                                            clickTimeoutRef.current = setTimeout(() => {
                                                ...
                                                setTabs([
                                                    ...
                                                    { 
                                                        ...
                                                        value: { 
                                                            icon: value.name === "overview" ? <Inbox className="w-4 h-4 text-gray-600/80" /> : value.name === "calendar" ? <Calendar1 className="w-4 h-4 text-gray-600/80" /> : value.name === "study tracker" ? <Timer className="w-4 h-4 text-gray-600/80" /> : <Hash className="w-3 h-3 text-gray-600/80" />   
                                                        },
                                            ...
                                            { value.name === "overview" ? <Inbox className="w-4 h-4 text-gray-600/80"/> : value.name === "calendar" ? <Calendar1 className="w-4 h-4 text-gray-600/80" /> : value.name === "study tracker" ? <Timer className="w-4 h-4 text-gray-600/80" /> : <Hash className="w-4 h-4 text-gray-600/80" />  }
                                            {
                                                editing === index ? (
