...
export default function TitleBar() {
    ...
    return (
        <div className="relative w-full h-8 flex flex-row items-center justify-between bg-[#EDEDF2] shadow-sm z-0">
            <div data-tauri-drag-region className="flex absolute top-0 left-0 w-full h-full"></div>
            <div className="group ml-1 hover:bg-gray-300 transition-all duration-200 py-0.5 hover:cursor-pointer select-none px-2 z-10 rounded-md">
            ...
            <div className="flex w-fit h-full z-10">
                ...
                <button id="titlebar-maximize" title={maximised ? "Restore" : "Maximise"} onClick={toggleMaximise} className="hover:bg-gray-300 h-full px-2 transition-all duration-200">
                    {
                        maximised ? <Copy className="w-3 h-3 rotate-90" /> : <Square className="w-4 h-4" />
                    }
                </button>
