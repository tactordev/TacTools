...
export const TimerProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const saveToEvent = (startTime: number, curType: SessionType) => {
    ...
    for (const idx of JSON.parse(rawCals)) {
      ...
      if (
        JSON.parse(rawCalInfo).title !==
        `${curType.name.toLowerCase()}-${curType.id}`
      )
        continue;
    ...
    if (!calNeeded) {
      const max =
        JSON.parse(rawCals).reduce(
          (m: number, i: string) => Math.max(m, Number(i)),
          0,
        ) + 1;
      localStorage.setItem(
        "loaded-calendars",
        JSON.stringify([...JSON.parse(rawCals), max]),
      );
      id = max;
    ...
    const newEvs = [
      ...curEvs.filter(
        (event: Event) => event.uid !== `${curType.id}-${startTime}`,
      ),
      newEv,
    ];
    localStorage.setItem(
