...
function generateRecurringEvents(
  ...
  const until =
    recurrenceUntil && recurrenceUntil.getTime() > startD.getTime()
      ? recurrenceUntil
      : defaultUntil;
      ...
      <div className="relative w-full h-[calc(100%-4rem)] mt-9">
        <AnimatePresence>
          {events.map((event: Event, index: number) => {
            ...
            const isInThisDay =
              eventTimestamp >= startOfDayTimestamp &&
              eventTimestamp <= endOfDayTimestamp;
            ...
            const heightPercent =
              ((event.end - event.start) / totalMinutes) * 100;
            ...
            return (
              ...
              >
                <p className="font-semibold text-gray-600 truncate leading-none">
                  {event.name}
                </p>
                <p className="text-[0.625rem] text-blue-700/80 mt-1">
                  {Math.floor(event.start / 60)}:
                  {(event.start % 60).toString().padStart(2, "0")}
                </p>
              </motion.div>
            );
          })}
    ...
    <div className="flex flex-col h-[calc(100%-4rem)] w-fit mt-1 justify-between">
      {times.map((time, index) => {
        ...
        return (
          <p
            key={`time-${index}`}
            className="text-gray-600/40 flex justify-center text-[11px] font-mono tabular-nums leading-none"
          >
            {formattedTime}
          </p>
        );
...
const NewEvent = ({
  ...
  const [dropdownChoice, setDropdownChoice] = useState<"calendar" | "event">(
    "event",
  );
  ...
  const filCals = cals.filter((cal: any) =>
    cal?.title?.toLowerCase().includes(input.toLowerCase()),
  );
  ...
  const createNewEvent = (e: React.MouseEvent) => {
    ...
    if (!title || !start || !end || !description || !calendar)
      return setDropdown(false);
    ...
    const baseUid = String(
      events.reduce(
        (max, cur) => Math.max(max, Number(cur.uid.split("-")[0]) || 0),
        0,
      ) + 1,
    );

    const untilDate =
      repeat !== "none" && repeatUntil ? new Date(repeatUntil) : undefined;
    ...
    setCals((prevCals: any) =>
      prevCals.map((cal: any) => {
        if (cal.id === calId) {
  ...
  const createNewCalendar = () => {
    ...
    const calendar = {
      ...
      id:
        cals.reduce(
          (max, cur) => Math.max(max, Number((cur as any).id || 0)),
          0,
        ) + 1,
      title: title,
      visible: true,
      ...
      <AnimatePresence>
        {dropdown && (
          <motion.div
            ...
            className="absolute flex flex-col top-full left-0 z-100 bg-gray-100/40 shadow-sm backdrop-blur-md mt-1 rounded-md transition-height duration-200"
          >
            <div className="flex flex-row gap-1 justify-around mx-1 px-1 py-1 mt-1 bg-gray-200 rounded-md backdrop-blur-md">
              <div
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setDropdownChoice("calendar");
                }}
                className={`px-8 py-0.5 rounded-md ${dropdownChoice === "calendar" ? "bg-white/60" : "opacity-20 hover:cursor-pointer hover:bg-white hover:opacity-40 transition-all duration-200"}`}
              >
                <LCalendar className="text-gray-500 w-4 h-4" />
              </div>
              <div
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setDropdownChoice("event");
                }}
                className={`px-8 py-0.5 rounded-md ${dropdownChoice === "event" ? "bg-white/60" : "opacity-20 hover:cursor-pointer hover:bg-white hover:opacity-40 transition-all duration-200"}`}
              >
                <Pin className="text-gray-500 w-4 h-4" />
              </div>
            </div>
            <AnimatePresence>
                ...
                >
                  ...
                  >
                    <div className="flex flex-row mt-1 relative items-center justify-center group">
                      ...
                      />
                      <div
                        className="w-fit h-fit absolute top-0.5 left-0 flex flex-row gap-1 items-center ml-2 pointer-events-none select-none 
                      ...
                      <AnimatePresence>
                        {searchDropdown && (
                          <motion.div
                            ...
                            className="absolute top-full mt-0.5 left-0 bg-gray-100 backdrop-blur-md rounded-md px-2 py-2 shadow-sm w-full h-[calc(100%+4rem)]"
                          >
                            <AnimatePresence>
                              {filCals.length === 0 ? (
                                <motion.p
                                ...
                                filCals.map((cal: any) => (
                                  ...
                                  >
                                    <Button
                                      onClick={() => {
                                        if (calSearchRef.current) {
                                          calSearchRef.current.focus();
                                          calSearchRef.current.value =
                                            cal.title;
                                          calSearchRef.current.id = `${cal.id}`;
                                        }
                                      }}
                                      name={`${cal.title}`}
                    ...
                    <div className="flex flex-col gap-1 mt-2.5">
                      ...
                      <select
                        value={repeat}
                        onChange={(e) =>
                          setRepeat(e.target.value as RecurrenceRules)
                        }
  ...
  return (
    <Button
      onClick={(e) => {
        setImportMenu({
          x:
            e.clientX + (e.currentTarget.getBoundingClientRect().x - e.clientX),
          y:
            e.clientY -
...
const EventContextMenu = ({
  ...
  id: string;
  setEventCtxMenu: (
    val: false | { x: number; y: number; eventId: string },
  ) => void;
}) => {
  ...
  const saveEvent = (e: React.MouseEvent) => {
    ...
    const isSameSeries = (uid: string) =>
      uid === baseId || uid.startsWith(`${baseId}-`);
    ...
    <div>
      {!editing ? (
        <div className="flex flex-col gap-1">
          ...
          <Button
            onClick={() => {
              if (confirmDelete !== "this-occurence")
                return setConfirmDelete("this-occurence");

              const evs = events.filter((e) => e.uid !== id);
              const calendars = cals.map((cal) => {
                return {
                  ...cal,
                  events: [...cal.events.filter((e: Event) => e.uid !== id)],
                };
              });
              setEvents(evs);
              setCals(calendars);
          ...
          >
            ...
            <p className={`text-red-500/60 text-xs min-w-20`}>
              {confirmDelete === "this-occurence"
                ? "Confirm?"
                : "Delete this occurence"}
            </p>
          </Button>
          <Button
            onClick={() => {
              if (confirmDelete !== "all-occurences")
                return setConfirmDelete("all-occurences");

              const evs = events.filter(
                (e) => e.uid.split("-")[0] !== id.split("-")[0],
              );
              const calendars = cals.map((cal) => {
                return {
                  ...cal,
                  events: [
                    ...cal.events.filter(
                      (e: Event) => !e.uid.includes(id.split("-")[0]),
                    ),
                  ],
                };
              });
                          ...
                          >
                            <Button
                              onClick={() => {
                                if (calSearchRef.current) {
                                  calSearchRef.current.focus();
                                  calSearchRef.current.value = cal.title;
                                  calSearchRef.current.id = `${cal.id}`;
                                }
                              }}
                              name={`${cal.title}`}
...
export default function Calendar() {
  ...
  const [infoCtxMenu, setInfoCtxMenu] = useState<
    { x: number; y: number } | false
  >(false);
  const [eventCtxMenu, setEventCtxMenu] = useState<
    { x: number; y: number; eventId: string } | false
  >(false);
  ...
  return (
    <div
      ...
      onContextMenu={(e) => {
        e.preventDefault();
        e.stopPropagation();
        setInfoCtxMenu({ x: e.clientX, y: e.clientY });
        setEventCtxMenu(false);
      }}
      className="flex flex-row w-full h-full gap-0.5 px-4 my-4"
    >
        ...
        {(eventCtxMenu || infoCtxMenu) && (
          <ContextMenu
            x={
              eventCtxMenu
                ? eventCtxMenu.x
                : (infoCtxMenu as { x: number; y: number }).x
            }
            y={
              eventCtxMenu
                ? eventCtxMenu.y
                : (infoCtxMenu as { x: number; y: number }).y
            }
            onBlur={() => {
              setEventCtxMenu(false);
              setInfoCtxMenu(false);
          ...
          >
            {eventCtxMenu ? (
              <div>
                <EventContextMenu
                  setEventCtxMenu={setEventCtxMenu}
                  events={events}
                  setEvents={setEvents}
                  cals={cals}
                  setCals={setCals}
                  id={eventCtxMenu.eventId}
                />
              </div>
            ) : (
              <div className="flex flex-col gap-1">
