...
function MDEditor({ tab }: { tab: Tab }) {
  const h1 = Decoration.line({
    attributes: { class: "text-xl font-extrabold block my-2" },
  });
  const h2 = Decoration.line({
    attributes: { class: "text-lg font-bold block my-1.5" },
  });
  const h3 = Decoration.line({
    attributes: { class: "text-base font-semibold block my-1" },
  });
  const h4 = Decoration.line({
    attributes: {
      class: "text-sm font-bold block my-1 text-gray-800 dark:text-gray-200",
    },
  });
  const h5 = Decoration.line({
    attributes: {
      class: "text-xs font-bold block my-1 text-gray-700 dark:text-gray-300",
    },
  });
  const h6 = Decoration.line({
    attributes: {
      class:
        "text-xs font-semibold block my-1 text-gray-600 dark:text-gray-400 italic",
    },
  });

  const multilineCode = Decoration.line({
    attributes: {
      class:
        "bg-gray-100 dark:bg-gray-800 font-mono text-sm block px-4 py-2 my-2 rounded-md",
    },
  });
  const bulletPoints = Decoration.line({
    attributes: { class: "list-disc pl-5" },
  });
  const ol = Decoration.line({ attributes: { class: "list-decimal pl-5" } });
  const indentBlock = Decoration.line({
    attributes: {
      class: "border-l-4 border-blue-300 dark:border-gray-600 pl-4 italic my-2",
    },
  });

  const italic = Decoration.mark({ attributes: { class: "italic" } });
  const bold = Decoration.mark({ attributes: { class: "font-bold" } });
  const strikethrough = Decoration.mark({
    attributes: { class: "line-through text-gray-400 dark:text-gray-500" },
  });
  const inlineCode = Decoration.mark({
    attributes: {
      class:
        "bg-gray-100 dark:bg-gray-800 text-red-500 dark:text-red-400 font-mono text-xs px-1 py-0.5 rounded",
    },
  });
  ...
  const image = Decoration.mark({
    attributes: { class: "text-green-600 font-medium" },
  });

  const hr = Decoration.line({
    attributes: {
      class:
        "border-t border-gray-300 dark:border-gray-700 my-4 h-0 block w-full",
    },
  });

  const mathBlockDeco = Decoration.line({
    attributes: {
      class:
        "bg-blue-50/30 dark:bg-blue-950/20 font-mono text-center block py-2 my-2 rounded-md border-y border-blue-100/30 dark:border-blue-900/20",
    },
  });
  const mathTextDeco = Decoration.mark({
    attributes: { class: "text-blue-600 dark:text-blue-400 font-mono italic" },
  });

  const hideSyntax = Decoration.mark({
    attributes: { class: "cm-hidden-syntax" },
  });
    ...
    case "gif":
      return (
        <div className="flex flex-row items-center justify-center w-full h-full">
          <img
            src={convertFileSrc(tab.value.path.replace("\\\\", "\\"))}
            className="mx-4 my-2 w-204 h-auto"
          />
        </div>
      );
